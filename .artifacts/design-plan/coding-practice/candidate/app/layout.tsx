 ...
 * fixture locale `vi`. The contact copy this case renders comes from the FIXTURE rather than from
 * the catalogue - `src/messages/vi.json` has no `contact` namespace yet, because the route it
 * would serve does not exist - so the catalogue is mounted for the shared leaves that read it and
 * every rendered state declares `locale: vi` instead of claiming locale coverage it does not have.
 */
