...
export type LayoutClassName =
    ...
    // statement read at that width wraps every second word. The seam flips with the axis - a rule
    // UNDER the reading column while the two are stacked, and BESIDE it once they are side by side -
    // which is why the reset comes with it rather than being left to look like an oversight.
