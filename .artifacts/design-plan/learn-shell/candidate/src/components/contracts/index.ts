...
export const CONTRACTS = buildContracts({
    /*
     * PROPOSED - the learn spine, direction A. Target path on materialization: the locked table.
     *
     * INVENTORY, taken before any of these was written:
     * `profile-rail-then-main` and `main-then-rail` are PAGE rails - they scroll with the page and
     * end where the content ends. A learn spine is app furniture: it stays under the navbar, takes
     * the viewport's remaining height, and scrolls on its own while the reading scrolls past it.
     * Neither existing key can say that, so the frame is NEW.
     * `marked-row-list`, `course-module-list` and `quick-actions-list` are all JOINED CARD lists -
     * dividers, a card inset, a surface. A nav group is a label over destinations with no surface
     * at all, so the group is NEW rather than a fourth joined list.
     */
