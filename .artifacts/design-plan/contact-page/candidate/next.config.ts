...
const nextConfig: NextConfig = {
    ...
    // Only for the EXPORT. `next dev` serves from the root, so pinning the path in both modes
    // would make every dev URL 404 - and the review lab embeds the exported build, not the dev
    // server, so the two genuinely want different roots.
    ...(process.env.NODE_ENV === "production"
        ? { basePath: "/candidate/out", assetPrefix: "/candidate/out" }
        : {}),
}
