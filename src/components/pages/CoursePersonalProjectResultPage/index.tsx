...
export const CoursePersonalProjectResultPage = ({ displayId, taskId }: CoursePersonalProjectResultPageProps) => {
    ...
    const attempts = useQueryPersonalTaskAttemptsSwr(project.data?.course.id, taskId, historyPage)
    const [selectedAttemptId, setSelectedAttemptId] = useState<string>()
    const selectedAttempt = attempts.data?.data.find((attempt) => attempt.id === selectedAttemptId)
        ?? attempts.data?.data[0]
    ...
    useEffect(() => {
        const first = attempts.data?.data[0]
        if (first !== undefined && !attempts.data?.data.some((attempt) => attempt.id === selectedAttemptId)) {
            setSelectedAttemptId(first.id)
        }
    }, [attempts.data, selectedAttemptId])
    ...
    const selected = rows.find((attempt) => attempt.id === selectedAttempt?.id)

    return <CoursePersonalProjectResultPageBase
        state={state}
        props={{
            ...
            attempts: rows,
            attemptCount: attempts.data?.count ?? 0,
        ...
        on={{
            ...
            previousHistoryPage: () => setHistoryPage((page) => Math.max(0, page - 1)),
            nextHistoryPage: () => setHistoryPage((page) => page + 1),
        }}
    />
}
