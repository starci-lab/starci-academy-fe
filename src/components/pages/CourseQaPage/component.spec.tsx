...
import { CourseQaBase } from "@/components/blocks/learn/CourseQa/component"
...
describe("CourseQaPage", () => {
    it("renders the course question list and the real ask control", () => {
        render(<CourseQaBase state="ready" props={props} />)
    ...
    it("offers the way back from a failed board", () => {
        const retry = vi.fn()
        render(<CourseQaBase state="failed" props={props} on={{ retry }} />)
    ...
    it("says nothing matched rather than drawing an empty surface as a result", () => {
        render(<CourseQaBase state="ready" props={{ ...props, questions: [] }} />)
