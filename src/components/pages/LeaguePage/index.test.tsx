...
const m = vi.hoisted(() => ({ token: "token" as string | undefined, restoring: false, weekly: { data: undefined as any, error: undefined as any, mutate: vi.fn() }, global: { data: undefined as any }, me: { data: { username: "me", avatar: null } }, push: vi.fn(), replace: vi.fn(), follow: vi.fn().mockResolvedValue({ data: { setFollow: { success: true } } }) }))
...
vi.mock("./component", () => ({ _LeaguePage: ({ state, on }: any) => <><output data-testid="state">{state}</output><button onClick={() => on.selectScope("global")}>global</button><button onClick={on.goHome}>home</button><button onClick={on.climb}>climb</button><button onClick={on.retry}>retry</button></> }))
