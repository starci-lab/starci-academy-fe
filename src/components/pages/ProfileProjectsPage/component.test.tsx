...
describe("_ProfileProjectsPage", () => {
    it("keeps pinned project tiles before the verified capstone list", () => {
        const html = renderToStaticMarkup(<_ProfileProjectsPage pinned={{ state: "ready", data: [{ id: "pin", type: "external", title: "Design tokens", description: "Shared tokens", techStack: ["TypeScript"], orderIndex: 0, isVerified: false }] }} capstones={{ state: "ready", data: [capstone] }} on={{ openPinned: vi.fn(), openCapstone: vi.fn() }} />)
