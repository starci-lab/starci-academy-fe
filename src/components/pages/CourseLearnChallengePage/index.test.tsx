...
describe("CourseLearnChallengePage route", () => {
    it("renders loading while dependent challenge data is unresolved", () => { const view = render(<CourseLearnChallengePage displayId="course" contentId="content" moduleId="module" challengeId="challenge" />); expect(screen.getByTestId("state")).toHaveTextContent(/pending|loading|connecting/); m.error = new Error("offline"); view.rerender(<CourseLearnChallengePage displayId="course" contentId="content" moduleId="module" challengeId="challenge" />); expect(screen.getByTestId("state")).toHaveTextContent(/pending|loading|failed|error/) })
})
