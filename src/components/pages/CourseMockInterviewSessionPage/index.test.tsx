...
vi.mock("./component", () => ({ _CourseMockInterviewSessionPage: ({ state, on }: any) => <><output data-testid="state">{state}</output><button onClick={on.retry}>retry</button><button onClick={on.leave}>leave</button><button onClick={on.abort}>abort</button></> }))
