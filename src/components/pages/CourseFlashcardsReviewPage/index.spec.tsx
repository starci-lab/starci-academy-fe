...
vi.mock("./component", () => ({ CourseFlashcardsReviewPageBase: ({ state, on }: TestPageInput) => <><output data-testid="state">{state}</output><button onClick={on.openQuiz}>quiz</button><button onClick={on.startDue}>due</button><button onClick={() => on.startDeck("deck")}>deck</button><button onClick={on.retry}>retry</button></> }))
...
describe("CourseFlashcardsReviewPage route", () => {
    ...
    it("dispatches review actions after settling a deck", () => { m.data = { id: "course" }; m.decks.data = [{ id: "deck", title: "Deck", description: "", difficulty: "junior", cards: [{ id: "card" }], dueCount: 1, masteredCount: 0 }]; m.due.data = { dueCount: 1, cards: [{ cardId: "card" }] }; m.stats.data = { currentStreak: 2, retentionRate: 80 }; render(<CourseFlashcardsReviewPage displayId="course" />); fireEvent.click(screen.getByText("quiz")); fireEvent.click(screen.getByText("due")); fireEvent.click(screen.getByText("deck")); fireEvent.click(screen.getByText("retry")) })
})
