...
vi.mock("./component", () => ({ CourseFlashcardsReviewPageBase: ({ state, on }: TestPageInput) => <><output data-testid="state">{state}</output><button onClick={on.openQuiz}>quiz</button><button onClick={() => on.openReview()}>due</button><button onClick={() => on.openReview("deck")}>deck</button><button onClick={() => on.openReview("missing")}>missing</button><button onClick={on.confirmReview}>confirm</button><button onClick={on.retry}>retry</button></> }))
