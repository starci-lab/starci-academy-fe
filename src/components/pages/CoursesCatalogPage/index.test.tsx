...
const m = vi.hoisted(() => ({ catalog: { data: undefined as any, error: undefined as any, mutate: vi.fn() }, mine: { data: undefined as any }, push: vi.fn() }))
...
vi.mock("@/components/overlays/courses/CoursePriceOverlay", () => ({ CoursePriceOverlay: ({ isOpen, onDismiss }: any) => isOpen ? <button onClick={onDismiss}>dismiss</button> : null }))
vi.mock("./component", () => ({ _CoursesCatalogPage: ({ state, on }: any) => <><output data-testid="state">{state}</output><button onClick={on.goHome}>home</button><button onClick={() => on.search("x")}>search</button><button onClick={() => on.changeView("line")}>line</button><button onClick={() => on.recover()}>recover</button><button onClick={() => on["view:c1"]?.()}>view</button></> }))
