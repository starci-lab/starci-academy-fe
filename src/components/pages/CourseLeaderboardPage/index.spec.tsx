...
const m = vi.hoisted(() => ({ scope: "challenge", course: { data: undefined as unknown, error: undefined as unknown, mutate: vi.fn() }, me: { data: undefined as unknown }, board: { data: undefined as unknown, error: undefined as unknown, mutate: vi.fn() }, push: vi.fn() }))
...
beforeEach(() => { vi.clearAllMocks(); m.scope = "challenge"; m.course.data = { id: "c1", title: "Course" }; m.course.error = undefined; m.me.data = { username: "me" }; m.board.data = undefined; m.board.error = undefined })
describe("CourseLeaderboardPage route", () => {
    ...
    it("renders a global board with a visible viewer row", () => { m.scope = "total"; m.me.data = { id: "me", username: "me" }; m.board.data = { entries: [{ rank: 1, username: "other", points: 100, userGlobalId: "other" }], myRank: { rank: 4, totalXp: 20, totalScore: 10, lessonsRead: 2, milestoneProgress: 1 }, computedAt: "2026-01-01" }; render(<CourseLeaderboardPage displayId="course" />); expect(screen.getByTestId("state")).toHaveTextContent("ready") })
})
