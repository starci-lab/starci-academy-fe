...
const m = vi.hoisted(() => ({ course: { data: undefined as any, error: undefined as any, mutate: vi.fn() }, me: { data: undefined as any }, board: { data: undefined as any, error: undefined as any, mutate: vi.fn() }, push: vi.fn() }))
...
vi.mock("./component", () => ({ _CourseLeaderboardPage: ({ state, on }: any) => <><output data-testid="state">{state}</output><button onClick={on.course}>course</button><button onClick={() => on.selectCategory("challenge")}>category</button><button onClick={on.climb}>climb</button><button onClick={on.retry}>retry</button></> }))
