...
vi.mock("./component", () => ({ CourseHeadhuntingsPageBase: ({ state }: TestPageInput) => <output data-testid="state">{state}</output> }))
...
describe("CourseHeadhuntingsPage route", () => {
    it("renders loading then failed transport states", () => { const view = render(<CourseHeadhuntingsPage displayId="course" />); expect(screen.getByTestId("state")).toHaveTextContent(/pending|loading|connecting/); m.error = new Error("offline"); view.rerender(<CourseHeadhuntingsPage displayId="course" />); expect(screen.getByTestId("state")).toHaveTextContent(/failed|error/) })
})
