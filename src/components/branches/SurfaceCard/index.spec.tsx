...
describe("SurfaceCard", () => {
    it("draws the ground alone for an object that names itself", () => {
        ...
        const owner = container.querySelector("[data-grammar-surface-card=\"true\"]")
        const surface = container.querySelector("[data-grammar-surface=\"true\"]")
        const content = container.querySelector("[data-grammar-surface-content=\"true\"]")
        ...
        expect(container.querySelectorAll("[data-grammar-contract=\"core.surface-card\"]")).toHaveLength(1)
    ...
    it("drops the inner surface for a section whose content is already made of cards", () => {
        ...
        const owner = container.querySelector("[data-grammar-surface-card=\"true\"]")
        const surface = container.querySelector(".starci-core-surface.starci-core-frameless-surface[data-grammar-surface=\"true\"][data-grammar-frame=\"frameless\"]")
        const content = container.querySelector("[data-grammar-surface-content=\"true\"]")
        expect(owner).not.toBe(surface)
        expect(surface).not.toBe(content)
        expect(owner?.contains(surface)).toBe(true)
    ...
    it("marks the one card that keeps its boundary while the rail scrolls inside it", () => {
        ...
        expect(container.querySelector("[data-component=\"SurfaceCardSurface\"]")).toHaveAttribute("data-grammar-scroll", "contained")
