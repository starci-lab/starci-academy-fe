        ...
        <PressableSurface label={data.title ?? "Course"} press={on?.open} disabled={isLoading || data.isPending === true || data.isDisabled === true} hover="label">
            <div className={courseProgressRowClassName}>
                ...
                <div className={courseProgressBodyClassName}>
                    <div className={courseProgressHeadingClassName}><Text size={"md"} weight={"semibold"} isSkeleton={isLoading}>{data.title}</Text>{data.isTrial === true && !isLoading ? <Badge tone={"neutral"}>{data.trialLabel}</Badge> : null}<Text size={"xs"} tone={"muted"} overflow={"wrap"} isSkeleton={isLoading}>{data.percentLabel}</Text></div>
                    ...
                    <div className={courseProgressLegendClassName}>{data.dimensions.map((dimension) => <span className={courseProgressLegendItemClassName} key={dimension.id}><StatusDot props={{ tone: dimension.tone, label: dimension.label }} isLoading={isLoading} /><Text size={"xs"} tone={"muted"} overflow={"wrap"} isSkeleton={isLoading}>{`${dimension.label} · ${dimension.completed}/${dimension.total}`}</Text></span>)}</div>
