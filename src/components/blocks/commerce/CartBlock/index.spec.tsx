...
type TestInput = { blockState: string; data: { payment: { isOpen: boolean; hasFailed: boolean; isPaying: boolean } }; on: { checkout: () => void; clearAll: () => void; goHome: () => void; browse: () => void; pay: () => void; dismissPayment: () => void } }
const mocks = vi.hoisted(() => ({
    ...
    clear: { trigger: vi.fn() }, checkout: { trigger: vi.fn(), isMutating: false }, cacheMutate: vi.fn(), push: vi.fn(),
}))
...
beforeEach(() => {
    ...
    mocks.checkout.isMutating = false
})
...
describe("CartBlock", () => {
    it("maps cart states and dispatches browse, clear and checkout actions", () => {
        ...
        expect(mocks.checkout.trigger).not.toHaveBeenCalled()
        act(() => { mocks.input?.on.pay() })
        expect(mocks.checkout.trigger).toHaveBeenCalledWith(expect.objectContaining({ paymentType: "payos" }))
    ...
    it("keeps the payment surface open and recoverable when PayOS cannot be opened", async () => {
        mocks.cart.data = [{ courseId: "c1", course: { title: "Course", coverImageUrl: null } }]
        mocks.preview.data = { lines: [{ courseId: "c1", chargedVnd: 80, listVnd: 100, discountPercent: 20 }], installmentOptions: [], totalListVnd: 100, savingsVnd: 20, totalChargedVnd: 80 }
        mocks.checkout.trigger.mockResolvedValueOnce({ data: { coursesCheckout: { data: { checkoutUrl: "" } } } })
        render(<CartBlock />)

        act(() => { mocks.input?.on.checkout() })
        act(() => { mocks.input?.on.pay() })
        await vi.waitFor(() => expect(mocks.input?.data.payment.hasFailed).toBe(true))
        expect(mocks.input?.data.payment.isOpen).toBe(true)
    })
})
