...
const catalogStateOf = (failed: boolean, pending: boolean, isEmpty: boolean, isSearching: boolean): CoursesCatalogBlockState => {
...
const catalogNoticeOf = (state: CoursesCatalogBlockState, t: (key: string) => string) => {
        ...
        <>
            <CoursesCatalogBlockBase
                ...
                props={{
                    ...
                    hasOwned,
                    pricedCourseId,
                    pricedCourseTitle: discover.find((course) => course.id === pricedCourseId)?.title,
                    discover, page, totalPages: Math.max(1, Math.ceil(count / PAGE_SIZE)), ...notice,
                }}
                on={{
                    search: (next: string) => {
                        setQuery(next)
                    ...
                    dismissPrice: () => setPricedCourseId(undefined),
                }}
            />
        </>
    )
