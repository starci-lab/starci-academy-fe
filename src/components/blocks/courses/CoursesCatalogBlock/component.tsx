...
/**
 * PAGE - `CoursesCatalogBlock`: the course catalog, grouped by whether the learner already owns it.
 *
 * Target path: `src/components/pages/CoursesCatalogBlock/component.tsx`.
 *
...
/** Presentation data owned by the connected catalog workspace. */
...
/** Props for {@link CoursesCatalogBlockBase}. */
 ...
 */
const CatalogLineListView = ({ props, on, isLoading = false }: LeafProps<CatalogLineListData, CoursesCatalogBlockActions>) => (
    <Tree
 ...
 * Draw the catalog.
 *
 * @param input - {@link CoursesCatalogBlockProps}
 */
export const CoursesCatalogBlockBase = (input: CoursesCatalogBlockProps) => {
    const labels = input.props.labels
    const isLoading = input.blockState === "pending"
    const discover = input.props.discover ?? []
    const showsNotice =
        input.blockState === "empty" || input.blockState === "filtered-empty" || input.blockState === "failed"
    ...
    return (
        <>
            ...
            />
            {input.props.pricedCourseId === undefined ? null : <CoursePriceOverlay courseId={input.props.pricedCourseId} title={input.props.pricedCourseTitle} isOpen onDismiss={input.on?.dismissPrice ?? (() => undefined)} />}
        </>
    )
}
