...
export const StarCiAiChatBase = (props: StarCiAiChatProps) => {
    ...
    const contextNode = isHistory || props.props.contextSummary === undefined ? null : <div className={aiChatClassNames.context} data-ai-context>
        <div className={aiChatClassNames.contextCopy}><span className={aiChatClassNames.contextDot} aria-hidden="true" /><Text size={"xs"} weight={"medium"}>{props.props.contextSummary}</Text></div>
        {selection === undefined ? null : <Button variant="ghost" size="sm" onPress={props.on?.clearContext}>{labels.clearContext}</Button>}
    </div>

    const transcript = (() => {
        if (isHistory && props.props.sessions.length > 0) return <div className={aiChatClassNames.history}>
            {props.props.sessions.map((session) => <Button key={session.id} variant={session.id === props.props.activeSessionId ? "secondary" : "ghost"} onPress={() => props.on?.selectSession?.(session.id)}>{`${session.title} · ${session.updatedLabel}`}</Button>)}
        ...
        if (showsEmpty) return <section className={aiChatClassNames.empty} aria-label={labels.emptyTitle}>
            ...
            <div className={aiChatClassNames.emptyCopy}><Heading level={2}>{labels.emptyTitle}</Heading><Text size={"sm"} tone={"muted"}>{labels.emptyDescription}</Text></div>
            <div className={aiChatClassNames.prompts}>{labels.quickPrompts.map((prompt) => <Button key={prompt} variant="outline" size="sm" onPress={() => props.on?.usePrompt?.(prompt)}>{prompt}</Button>)}</div>
        </section>
    ...
    const sendOrStop = props.state === "streaming"
        ? <Button variant="secondary" size="sm" onPress={props.on?.stop}>{labels.stop}</Button>
        : stateNeedsRetry(props.state)
            ? <Button variant="primary" size="sm" onPress={props.on?.retry}>{labels.retry}</Button>
            : <Button variant="primary" size="sm" isDisabled={disablesComposer || props.props.draft.trim() === ""} onPress={props.on?.send}>{labels.send}</Button>

    return <div className={aiChatClassNames.root} data-starci-ai-chat>
        <header className={aiChatClassNames.intro}><span className={aiChatClassNames.introAvatar}><StarCiAiTeacher props={{ size: "md", isOnline: true }} /></span><div className={aiChatClassNames.introCopy}><Text size={"sm"} weight={"semibold"}>{labels.eyebrow}</Text><Text size={"xs"} tone={"muted"}>{labels.subtitle}</Text></div></header>
        <nav className={aiChatClassNames.modes} aria-label={labels.eyebrow}><Button variant={props.props.mode === "general" ? "primary" : "ghost"} size="sm" onPress={() => props.on?.selectMode?.("general")}>{labels.generalMode}</Button><Button variant={props.props.mode === "history" ? "primary" : "ghost"} size="sm" onPress={() => props.on?.selectMode?.("history")}>{labels.historyMode}</Button></nav>
        {contextNode}
        ...
        {showsSessionActions ? <div className={aiChatClassNames.historyActions}>{props.state === "deleteConfirm" ? <><Button variant="primary" size="sm" onPress={props.on?.confirmDelete}>{labels.confirmDelete}</Button><Button variant="ghost" size="sm" onPress={props.on?.cancelDelete}>{labels.cancel}</Button></> : <><Button variant="ghost" size="sm" isPending={props.state === "renaming"} onPress={props.on?.rename}>{labels.rename}</Button><Button variant="ghost" size="sm" isPending={props.state === "archiving"} onPress={props.on?.archive}>{labels.archive}</Button><Button variant="ghost" size="sm" onPress={props.on?.delete}>{labels.delete}</Button></>}</div> : null}
        {showsComposer ? <div className={aiChatClassNames.composer}>
            ...
            <div className={aiChatClassNames.quota}>{props.state === "quotaPending" || props.props.quotaLabel !== undefined ? <Text size={"xs"} tone={"muted"} live={"polite"} isSkeleton={props.state === "quotaPending"}>{props.props.quotaLabel}</Text> : null}</div>
        </div> : null}
    </div>
}
