...
import { leagueRankedRowClassName, leagueStandingClassName } from "./classNames"
...
export const LeagueCardBase = (props: LeagueCardProps) => {
    ...
    return <SurfaceCard props={{ label: props.props.label, seeMoreLabel: props.props.seeMoreLabel }} on={{ seeMore: props.on?.seeMore }} isLoading={loading}>
        ...
        <SurfaceListCard props={{ label: props.props.label, isNested: true, isLabelHidden: true, isVerdict: rows.some((row) => row.verdict !== undefined) }} isLoading={loading}>
            {settledEmpty ? <EmptyNotice props={{ icon: "league", message: props.state === "empty" ? props.props.emptyMessage ?? "" : props.props.errorMessage ?? "", actionLabel: props.state === "failed" ? props.props.retryLabel : undefined }} on={{ act: props.state === "failed" ? props.on?.retry : undefined }} /> : rows.map((row) => <div className={leagueRankedRowClassName} data-dashboard-community-ranked-row="true" key={row.id}><RankedUserRow props={row} on={{ open: props.on?.[`open:${row.id}`] }} isLoading={loading} /></div>)}
        </SurfaceListCard>
    </SurfaceCard>
}
