...
export const MyCoursesProgressBase = (props: MyCoursesProgressProps) => { if (props.state === "empty" || props.state === "failed") return <SurfaceCard props={{ label: props.props.label }}><EmptyNotice props={{ icon: "course", message: props.state === "empty" ? props.props.emptyMessage ?? "" : props.props.errorMessage ?? "", actionLabel: props.props.retryLabel }} on={{ act: props.on?.retry }} /></SurfaceCard>; const rows = props.state === "pending" ? [] : props.props.rows; return <SurfaceListCard props={props.props} on={{ act: props.on?.act }} isLoading={props.state === "pending"}><div className={courseProgressListClassName}>{rows.map((row) => <CourseProgressRow key={row.id} props={row} on={{ open: props.on?.[`open:${row.id}`] }} isLoading={props.state === "pending"} />)}</div></SurfaceListCard> }
