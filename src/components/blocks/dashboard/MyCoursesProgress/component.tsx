...
import { courseProgressListClassName, courseProgressListRowClassName } from "./classNames"
...
export const MyCoursesProgressBase = (props: MyCoursesProgressProps) => {
    ...
    return <SurfaceListCard {...frame}><div className={courseProgressListClassName}>{rows.map((row) => <div className={courseProgressListRowClassName} key={row.id}><CourseProgressRow props={row} on={{ open: props.on?.[`open:${row.id}`] }} isLoading={isLoading} /></div>)}</div></SurfaceListCard>
}
