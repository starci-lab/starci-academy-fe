...
export const WeeklyChallengeCardBase = (props: WeeklyChallengeCardProps) => {
    ...
    return <SurfaceCard props={{ label: props.props.label }} isLoading={loading}><div><Icon props={{ name: "practice", role: "leading" }} /><Text props={{ content: props.props.title, size: "sm" }} isLoading={loading} /></div><div><Text props={{ content: props.props.endsInLabel, size: "xs", tone: "muted" }} isLoading={loading} />{props.props.claimed === true ? <Badge props={{ content: props.props.claimedLabel ?? "", tone: "success" }} /> : <Button props={{ label: props.props.actionLabel ?? "", size: "sm", variant: "primary", isPending: props.props.isClaiming }} on={{ press: props.on?.act }} isLoading={loading} />}</div>{finishers.length === 0 ? <Text props={{ content: props.props.passedCountLabel, size: "sm", tone: "muted" }} /> : <SurfaceListCard props={{ label: props.props.passedCountLabel ?? "", isNested: true }}>{finishers.map((finisher) => <div key={finisher.id}><Avatar props={{ name: finisher.label, size: "sm" }} isLoading={loading} /><Text props={{ content: finisher.label, size: "sm" }} isLoading={loading} /><Text props={{ content: finisher.passedAtLabel, size: "xs", tone: "muted" }} isLoading={loading} /></div>)}</SurfaceListCard>}</SurfaceCard>
}
