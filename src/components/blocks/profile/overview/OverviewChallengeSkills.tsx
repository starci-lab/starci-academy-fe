...
export const OverviewChallengeSkills = () => {
    ...
    challenges.forEach((item) => { if (item.difficulty) difficulty.set(item.difficulty, (difficulty.get(item.difficulty) ?? 0) + 1); if (item.selectedLang) languages.add(item.selectedLang) })
    ...
    return <SkillSnapshot label={t("overview.challengeSkills")} totalLabel={t("overview.passed")} totalValue={request.isLoading ? undefined : String(challenges.length)} rows={rows} isLoading={request.isLoading} stateMessage={request.error ? t("evidence.error") : !request.isLoading && challenges.length === 0 ? t("evidence.solved-challenges.empty") : languages.size ? t("overview.languages", { count: languages.size, list: [...languages].join(" · ") }) : undefined} />
}
