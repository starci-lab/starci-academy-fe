...
export const ProfileCvBuilderBase = (props: ProfileCvBuilderProps) => {
    ...
    return <div className={cvBuilderClassName}>
        <header className={cvBuilderHeaderClassName}>
            <div className={cvBuilderTitleClassName}><Text size={"xs"}>{"CV BUILDER · LATEX"}</Text><Heading level={2}>{document.label}</Heading><Text size={"sm"} tone={"muted"}>{"Biến dữ liệu thật thành một CV rõ ràng, dễ đọc và dễ qua ATS."}</Text><Text size={"xs"} startContent={<Icon source={iconSourceFor(data.saveState === "error" ? "retry" : "complete", "chip")} role="chip" />}>{saveLabel}</Text></div> {/* vn-ok: localized Vietnamese interface copy. */}
            <div className={cvBuilderActionsClassName}><Button variant="primary" isPending={data.isCompiling} onPress={on?.compile}>{"Biên dịch PDF"}</Button>{data.previewUrl && <a href={data.previewUrl} target="_blank" rel="noreferrer" className={cvBuilderPdfLinkClassName}><Icon source={iconSourceFor("explore", "chip")} role={"chip"} />Mở PDF</a>}</div> {/* vn-ok: localized Vietnamese interface copy. */}
        </header>
