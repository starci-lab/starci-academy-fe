...
/** Public challenge detail data rendered by the pure submission block. */
...
/** Pure block state and return action. */
export type ProfileChallengeSubmissionBlockProps = { readonly state: "pending" | "ready" | "error"; readonly detail?: ChallengeDetail | null; readonly onBack: () => void }

/** Draw immutable proof, attempts and structured feedback inside profile-main. */
export const ProfileChallengeSubmissionBase = ({ state, detail, onBack }: ProfileChallengeSubmissionBlockProps) => {
    ...
    return <Tree contract="profile-main" render={defineContractComponent("profile-main", { section: [
        ...
        ...(detail?.submissionUrl || state === "pending" ? [defineContractProjection("label-row-over-card", () => <SurfaceCard props={{ label: "Submitted proof" }} contract="profile-meta-list" render={defineContractComponent("profile-meta-list", { item: [defineLeafComponent("link", {}, () => <Link props={{ label: detail?.submissionUrl ?? "Loading proof", externalHref: detail?.submissionUrl ?? undefined }} isLoading={state === "pending"} />)] })} />)] : []),
        defineContractProjection("label-row-over-card", () => <SurfaceCard props={{ label: "Attempts" }} contract="profile-evidence-list" render={defineContractComponent("profile-evidence-list", { evidence: attempts.length > 0 ? attempts.map((attempt, index) => defineCompositeComponent("evidence-row", {}, () => <EvidenceRow props={{ title: `Attempt ${attempt.attemptNumber ?? index + 1}${index === 0 && (attempt.score ?? 0) > 0 ? " · Passed" : ""}`, subtitle: [attempt.processedAt, attempt.shortFeedback].filter(Boolean).join(" · "), fact: attempt.score == null ? undefined : String(attempt.score), factTone: (attempt.score ?? 0) >= 80 ? "success" : "neutral" }} isLoading={state === "pending"} />)) : [defineCompositeComponent("evidence-row", {}, () => <EvidenceRow props={{ title: "No public attempts were found." }} />)] })} />),
        defineContractProjection("label-row-over-card", () => <SurfaceCard props={{ label: "Structured feedback" }} contract="profile-evidence-list" render={defineContractComponent("profile-evidence-list", { evidence: feedbacks.length > 0 ? feedbacks.map((feedback, index) => defineCompositeComponent("evidence-row", {}, () => <EvidenceRow props={{ title: feedback.message ?? `Feedback ${index + 1}`, subtitle: feedback.detail ?? feedback.suggestion, fact: feedback.severity, factTone: feedback.severity?.toLowerCase().includes("strong") ? "success" : "warning" }} isLoading={state === "pending"} />)) : [defineCompositeComponent("evidence-row", {}, () => <EvidenceRow props={{ title: "No structured feedback was published." }} />)] })} />),
    ] })} />
}

/** Source-level ownership marker for the pure submission block. */
