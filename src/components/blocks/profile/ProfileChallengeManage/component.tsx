...
/** Connected challenge-management data rendered by this pure profile block. */
export type ProfileChallengeManageBlockProps = { readonly state: "pending" | "ready" | "error"; readonly courseTitle?: string; readonly rows: ReadonlyArray<ProfileSolvedChallenge>; readonly query: string; readonly filterLabel: string; readonly on: { readonly back: () => void; readonly search: (value: string) => void; readonly filter: () => void; readonly select: (id: string) => void } }

/** Draw course proof orientation, local search/filter controls and passed submissions. */
export const ProfileChallengeManageBase = ({ state, courseTitle, rows, query, filterLabel, on }: ProfileChallengeManageBlockProps) => {
...
/** Source-level ownership marker for the pure challenge-management block. */
