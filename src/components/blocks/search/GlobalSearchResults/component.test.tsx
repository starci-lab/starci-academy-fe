...
import { _GlobalSearchResults } from "./component"
...
describe("_GlobalSearchResults", () => {
    it("uses one label-less nested SurfaceListCard when results exist", () => {
        render(<_GlobalSearchResults props={{ label: "Results", items: [{ id: "one", textValue: "One", title: "One" }], emptyMessage: "Empty" }} />)
    ...
    it("selects a result for detail without exposing row activation", () => {
        const select = vi.fn()
        render(<_GlobalSearchResults props={{ label: "Results", items: [{ id: "one", textValue: "One", title: "One" }], emptyMessage: "Empty" }} on={{ select }} />)
    ...
    it("replaces the list surface with EmptyNotice when no result exists", () => {
        const recover = vi.fn()
        render(<_GlobalSearchResults props={{ label: "Results", items: [], emptyMessage: "Empty", emptyActionLabel: "Retry" }} on={{ recover }} />)
