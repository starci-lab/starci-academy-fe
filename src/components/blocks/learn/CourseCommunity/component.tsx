...
export const CourseCommunityBase = (props: CourseCommunityProps) => {
    ...
    const feed = <div className={communityPrimaryClassName}>
        {fatalNotice ? <div className={communityFatalClassName}><SurfaceCard label={props.state === "failed" ? props.copy.failedTitle : props.copy.forbiddenTitle} composition="joined"><EmptyNotice message={props.state === "failed" ? props.copy.failedLabel : props.copy.forbiddenLabel} actionLabel={props.state === "failed" ? props.copy.retryLabel : props.copy.openCourseLabel} iconSource={iconSourceFor(props.state === "failed" ? "retry" : "community", "leading")} actionStartContent={<Icon source={iconSourceFor(props.state === "failed" ? "retry" : "back", "chip")} role="chip" />} onAction={({ act: props.state === "failed" ? props.on.retry : props.on.openCourse })?.act} /></SurfaceCard></div> : <>
            {composer}
