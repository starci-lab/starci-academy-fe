...
export const PersonalProjectTaskBase = (props: PersonalProjectTaskProps) => {
    ...
    const brief = sections.map((section, index) => {
        ...
        return <SurfaceCard key={section.id} props={{ label: section.label, inset: "compact" }} isLoading={isLoading}><Article props={{ body: section.body }} isLoading={isLoading} />{index === 0 && props.props.hint !== undefined ? <Text props={{ content: props.props.hint, size: "sm", tone: "muted", icon: "review" }} /> : null}</SurfaceCard>
    })
    ...
    return (
        <div className={personalProjectTaskClassName}>
            ...
            <main className={personalProjectTaskGridClassName}>
                <div className={personalProjectTaskBriefClassName} data-active={activeView === "content"}>{briefContent}</div>
