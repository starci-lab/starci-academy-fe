...
export const PersonalProjectTaskBase = (props: PersonalProjectTaskProps) => {
    ...
    // The way back out of a task is the personal-project page itself, so with a resolved route it is
    // an anchor and keeps middle-click and copy-link; without one it stays the press it was.
    const backAction = (appearance: "inline" | "muted") => props.props.backHref === undefined
        ? <TextAction appearance={appearance} startContent={<Icon source={iconSourceFor("back", "chip")} usage="chip" />} onPress={props.on?.back}>{props.props.labels.back}</TextAction>
        : <TextAction appearance={appearance} href={props.props.backHref} startContent={<Icon source={iconSourceFor("back", "chip")} usage="chip" />} onFollow={props.on?.back}>{props.props.labels.back}</TextAction>
    const briefContent = props.state === "forbidden" ? <SurfaceCard label={props.props.labels.lockedTitle} composition="single">{status}{backAction("inline")}</SurfaceCard> : props.state === "task-error" ? <SurfaceCard composition="single">{status}<Button variant="primary" onPress={props.on?.retry}>{props.props.labels.retry}</Button></SurfaceCard> : <>{brief}{implementation}{criteria}</>
