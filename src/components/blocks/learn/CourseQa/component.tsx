...
type CourseQaBlockProps = BlockProps<"pending" | "ready" | "empty" | "failed", CourseQaPageData> & {
    readonly on?: CourseQaPageActions
}

/** Board transport/collection states belong to the question-list block, not the page frame. */
...
export const CourseQaBase = (input: CourseQaBlockProps) => {
    const questionState = input.state
    const isLoading = questionState === "pending"
    ...
    const notice = questionState === "failed" || questionState === "empty"
        ? defineCompositeComponent("empty-notice", {}, () => (
            <EmptyNotice
                props={{
                    icon: questionState === "failed" ? "retry" : "community",
                    message: questionState === "failed" ? input.props.errorMessage : input.props.emptyMessage,
                    actionLabel: questionState === "failed" ? input.props.retryLabel : undefined,
                }}
                on={{ act: questionState === "failed" ? input.on?.retry : undefined }}
            />
        ))
        : undefined

    return (
        <Tree contract="course-qa-workspace" render={defineContractComponent("course-qa-workspace", {
            header,
            composer: toolbar,
            ...(notice === undefined ? {
