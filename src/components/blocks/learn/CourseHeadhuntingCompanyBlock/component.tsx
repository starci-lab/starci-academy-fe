...
export const CourseHeadhuntingCompanyBlockBase = (input: CourseHeadhuntingCompanyPageProps) => {
    ...
    const noticeMessage = input.blockState === "not-found" ? input.props.notFoundMessage : input.props.errorMessage
    const notice = input.blockState === "not-found" || input.blockState === "failed"
        ? defineCompositeComponent("empty-notice", {}, () => (
            <EmptyNotice
                props={{
                    icon: input.blockState === "failed" ? "retry" : "talents",
                    message: noticeMessage,
                    actionLabel: input.blockState === "failed" ? input.props.retryLabel : input.props.backLabel,
                }}
                on={{ act: input.blockState === "failed" ? input.on?.retry : input.on?.back }}
            />
        ))
        : undefined
