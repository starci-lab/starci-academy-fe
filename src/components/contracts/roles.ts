 ...
 */
export type ContractSlot = ComponentType<ContractSlotProps>

/**
 * The host elements a registry key may render as - a closed set, so a key cannot reach for an
 * arbitrary tag. Everything here is either a landmark, a list, or the neutral default.
 *
 * `ul` is here for the same reason `nav` is. A run of columns that is really a LIST - the seven
 * days of a streak - tells assistive technology how many items there are and which one it is on,
 * and that fact belongs to the KEY that lays the run out rather than to whoever fills it. Without
 * it the only way to reach a list is to write `<ul>` by hand, which the registry exists to refuse.
 */
...
export interface ContractSpec {
     ...
     */
    readonly element?: ContractElement
}
