...
import {
    ...
    mutationSignUpResendOtp,
    mutationSignUpResendOtpMap,
} from "./mutation-sign-up-resend-otp"
...
const documentText = print(mutationSignUpResendOtpMap[MutationSignUpResendOtp.Mutation1])
...
describe("mutationSignUpResendOtpMap", () => {
    it("names the operation so it is identifiable in a server log", () => {
        expect(documentText).toContain("mutation SignUpResendOtp")
    })

    it("declares the argument type the live schema declares", () => {
        expect(documentText).toContain("$request: SignUpResendOtpRequest!")
        expect(documentText).toContain("signUpResendOtp(request: $request)")
    })
...
describe("mutationSignUpResendOtp", () => {
    ...
    it("passes the abort signal and extra headers through to the client", async () => {
        ...
        await mutationSignUpResendOtp({ request, signal: controller.signal, headers: { "X-Locale": "en" } })
    ...
    it("returns whatever the client returned, unchanged", async () => {
        ...
        await expect(mutationSignUpResendOtp({ request })).resolves.toBe(result)
    })
})
