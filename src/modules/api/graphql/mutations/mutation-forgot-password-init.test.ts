...
import {
    MutationForgotPasswordInit,
    mutationForgotPasswordInit,
    mutationForgotPasswordInitMap,
} from "./mutation-forgot-password-init"
...
const documentText = print(mutationForgotPasswordInitMap[MutationForgotPasswordInit.Mutation1])

/** The request this operation is exercised with. */
const request = { email: "learner@example.com", newPassword: "secret" }
...
describe("mutationForgotPasswordInitMap", () => {
    it("names the operation so it is identifiable in a server log", () => {
        expect(documentText).toContain("mutation ForgotPasswordInit")
    })

    it("declares the argument type the live schema declares", () => {
        expect(documentText).toContain("$request: ForgotPasswordInitRequest!")
        expect(documentText).toContain("forgotPasswordInit(request: $request)")
    })
...
describe("mutationForgotPasswordInit", () => {
    it("sends the document for the default variant with the request as its single argument", async () => {
        await mutationForgotPasswordInit({ request })
        expect(mocks.mutate).toHaveBeenCalledTimes(1)
        expect(mocks.mutate.mock.calls[0][0]).toEqual({
            mutation: mutationForgotPasswordInitMap[MutationForgotPasswordInit.Mutation1],
            variables: { request },
        })
    })
    ...
    it("passes the abort signal and extra headers through to the client", async () => {
        ...
        await mutationForgotPasswordInit({ request, signal: controller.signal, headers: { "X-Locale": "en" } })
    ...
    it("returns whatever the client returned, unchanged", async () => {
        ...
        await expect(mutationForgotPasswordInit({ request })).resolves.toBe(result)
    })
})
