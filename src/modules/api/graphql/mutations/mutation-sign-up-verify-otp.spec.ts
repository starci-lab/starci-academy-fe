...
import {
    ...
    mutationSignUpVerifyOtp,
    mutationSignUpVerifyOtpMap,
} from "./mutation-sign-up-verify-otp"
...
const documentText = print(mutationSignUpVerifyOtpMap[MutationSignUpVerifyOtp.Mutation1])
...
describe("mutationSignUpVerifyOtpMap", () => {
    it("names the operation so it is identifiable in a server log", () => {
        expect(documentText).toContain("mutation SignUpVerifyOtp")
    })

    it("declares the argument type the live schema declares", () => {
        expect(documentText).toContain("$request: SignUpVerifyOtpInput!")
        expect(documentText).toContain("signUpVerifyOtp(request: $request)")
    })
...
describe("mutationSignUpVerifyOtp", () => {
    ...
    it("passes the abort signal and extra headers through to the client", async () => {
        ...
        await mutationSignUpVerifyOtp({ request, signal: controller.signal, headers: { "X-Locale": "en" } })
    ...
    it("returns whatever the client returned, unchanged", async () => {
        ...
        await expect(mutationSignUpVerifyOtp({ request })).resolves.toBe(result)
    })
})
