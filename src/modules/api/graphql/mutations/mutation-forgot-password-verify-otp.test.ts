...
import {
    MutationForgotPasswordVerifyOtp,
    mutationForgotPasswordVerifyOtp,
    mutationForgotPasswordVerifyOtpMap,
} from "./mutation-forgot-password-verify-otp"
...
const documentText = print(mutationForgotPasswordVerifyOtpMap[MutationForgotPasswordVerifyOtp.Mutation1])

/** The request this operation is exercised with. */
const request = { challengeId: "challenge-1", otp: "123456" }
...
describe("mutationForgotPasswordVerifyOtpMap", () => {
    it("names the operation so it is identifiable in a server log", () => {
        expect(documentText).toContain("mutation ForgotPasswordVerifyOtp")
    })

    it("declares the argument type the live schema declares", () => {
        expect(documentText).toContain("$request: ForgotPasswordVerifyOtpRequest!")
        expect(documentText).toContain("forgotPasswordVerifyOtp(request: $request)")
    })
    ...
    it("selects exactly what this step produces", () => {
        ...
        expect(documentText).not.toContain("challengeId {")
    })
})

describe("mutationForgotPasswordVerifyOtp", () => {
    it("sends the document for the default variant with the request as its single argument", async () => {
        await mutationForgotPasswordVerifyOtp({ request })
        expect(mocks.mutate).toHaveBeenCalledTimes(1)
        expect(mocks.mutate.mock.calls[0][0]).toEqual({
            mutation: mutationForgotPasswordVerifyOtpMap[MutationForgotPasswordVerifyOtp.Mutation1],
            variables: { request },
        })
    })
    ...
    it("passes the abort signal and extra headers through to the client", async () => {
        ...
        await mutationForgotPasswordVerifyOtp({ request, signal: controller.signal, headers: { "X-Locale": "en" } })
    ...
    it("returns whatever the client returned, unchanged", async () => {
        ...
        await expect(mutationForgotPasswordVerifyOtp({ request })).resolves.toBe(result)
    })
})
