...
import {
    ...
    mutationExchangeCodeForToken,
    mutationExchangeCodeForTokenMap,
} from "./mutation-exchange-code-for-token"
import { KeycloakIdentityProvider } from "./types/auth"
...
const documentText = print(mutationExchangeCodeForTokenMap[MutationExchangeCodeForToken.Mutation1])

/** The request this operation is exercised with. */
const request = { code: "auth-code", provider: KeycloakIdentityProvider.Google, state: "state-1" }
...
describe("mutationExchangeCodeForTokenMap", () => {
    it("names the operation so it is identifiable in a server log", () => {
        expect(documentText).toContain("mutation ExchangeCodeForToken")
    })

    it("declares the argument type the live schema declares", () => {
        expect(documentText).toContain("$request: ExchangeCodeForTokenRequest!")
        expect(documentText).toContain("exchangeCodeForToken(request: $request)")
    })
...
describe("mutationExchangeCodeForToken", () => {
    it("sends the document for the default variant with the request as its single argument", async () => {
        ...
        expect(mocks.mutate.mock.calls[0][0]).toEqual({
            mutation: mutationExchangeCodeForTokenMap[MutationExchangeCodeForToken.Mutation1],
            variables: { request },
        })
    })
    ...
    it("returns whatever the client returned, unchanged", async () => {
        ...
        await expect(mutationExchangeCodeForToken({ request })).resolves.toBe(result)
    })
})
