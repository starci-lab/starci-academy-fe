...
import {
    MutationSignUpInit,
    mutationSignUpInit,
    mutationSignUpInitMap,
} from "./mutation-sign-up-init"
...
const documentText = print(mutationSignUpInitMap[MutationSignUpInit.Mutation1])

/** The request this operation is exercised with. */
const request = { email: "learner@example.com", password: "secret", username: "learner", firstName: "Ada", lastName: "Lovelace" }
...
describe("mutationSignUpInitMap", () => {
    it("names the operation so it is identifiable in a server log", () => {
        expect(documentText).toContain("mutation SignUpInit")
    })

    it("declares the argument type the live schema declares", () => {
        expect(documentText).toContain("$request: SignUpInitRequest!")
        expect(documentText).toContain("signUpInit(request: $request)")
    })
...
describe("mutationSignUpInit", () => {
    ...
    it("passes the abort signal and extra headers through to the client", async () => {
        ...
        await mutationSignUpInit({ request, signal: controller.signal, headers: { "X-Locale": "en" } })
    ...
    it("returns whatever the client returned, unchanged", async () => {
        ...
        await expect(mutationSignUpInit({ request })).resolves.toBe(result)
    })
})
