...
/** The house Input uses HeroUI's inset surface variant, not another bordered ground. */
export const inputUsesSecondaryVariant = {
  meta: {
    type: "problem",
    docs: { description: "The Input leaf uses the secondary input variant on a bounded surface." },
    schema: [],
    messages: {
      variant:
        "The house Input sits inside bounded form surfaces. Its HeroUI Input must use `variant=\"secondary\"`; the default variant draws a competing field surface.",
    },
  },
  create(context) {
    const file = normalizePath(context.filename || context.getFilename())
    if (!/\/src\/components\/leaves\/Input\/index\.tsx$/.test(file)) return {}
...
export const fieldLabelIsTextOnly = {
  ...
  create(context) {
    const file = normalizePath(context.filename || context.getFilename())
    if (!/\/src\/components\/composites\/Field\/index\.tsx$/.test(file)) return {}
    ...
    return {
      ImportDeclaration(node) {
        ...
        for (const specifier of node.specifiers || []) {
          if (specifier.imported?.name === "Icon" && specifier.local?.name) {
  ...
  create(context) {
    ...
    return {
      JSXOpeningElement(node) {
        ...
        if (name === "HeroCheckbox.Control" && hasAncestor(node, "HeroCheckbox.Content")) {
          controlInsideContent = true
        }
        if (name === "HeroCheckbox.Indicator" && hasAncestor(node, "HeroCheckbox.Control")) {
          indicatorInsideControl = true
        }
      },
      "Program:exit"(node) {
  ...
  create(context) {
    ...
    return {
      ...
      ImportDeclaration(node) {
        ...
        if (isNav && /\/components\/blocks\/auth\/AccountMenu$/.test(normalizePath(node.source?.value))) {
          hasOwner = true
        }
      },
      JSXOpeningElement(node) {
